# Updated main.py with language selection for multilingual ElevenLabs TTS
...
# ========== TTS FUNCTION ==========
def play_elevenlabs_tts(text, language):
    ...
    if language != "English":
        ...
        try:
            ...
            st.info(f"🌐 Translated to {language}: {text}")
        except Exception as e:
    ...
    with st.chat_message(role):
        ...
        if role == "assistant":
            ...
            if st.session_state.play_flags[msg_key] and st.session_state.speech_attempts[msg_key] < 2:
                ...
                audio = play_elevenlabs_tts(content, language)
                if audio:
